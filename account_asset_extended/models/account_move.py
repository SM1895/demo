...
class AccountMove(models.Model):
    ...
    asset_fiscal_id = fields.Many2one('account.asset', string='Activo', index=True, ondelete='cascade', copy=False, domain="[('company_id', '=', company_id)]")
    ...
    @api.depends(
        ...
        'asset_id',
        'depreciation_value',
        'asset_fiscal_id.total_depreciable_fiscal_value',
        'asset_id.total_depreciable_value',
        'asset_fiscal_id.already_depreciated_amount_fiscal_import',
    ...
    def _compute_depreciation_cumulative_value(self):
        ...
        if self.asset_fiscal_id:
            #self.asset_depreciated_value = 0
            #self.asset_remaining_value = 0

            for asset in self.asset_fiscal_id:
                depreciated = 0
                remaining = asset.total_depreciable_fiscal_value - asset.already_depreciated_amount_fiscal_import
                for move in asset.depreciation_fiscal_move_ids.sorted(lambda mv: (mv.date, mv._origin.id)):
    ...
    def _compute_depreciation_value(self):
        ...
        for move in self:
            if move.asset_fiscal_id:
                asset = move.asset_fiscal_id or move.reversed_entry_id.asset_fiscal_id  # reversed moves are created before being assigned to the asset
                if asset:
                    account = asset.account_depreciation_expense_fiscal_id if asset.asset_type != 'sale' else asset.account_depreciation_fiscal_id
                    ...
                    if any(
                        (line.account_id, -line.balance) == (asset.account_asset_fiscal_id, asset.original_fiscal_value)
                        for line in move.line_ids
                    ):
                        account = asset.account_depreciation_fiscal_id
                        asset_depreciation = (
                            asset.original_fiscal_value
                            - asset.salvage_fiscal_value
                            - sum(
                                move.line_ids.filtered(lambda l: l.account_id == account).mapped(
                                    'debit' if asset.original_fiscal_value > 0 else 'credit'
                                )
                            ) * (-1 if asset.original_fiscal_value < 0 else 1)
                        )
                else:
    ...
    # CONSTRAINT METHODS
    # -------------------------------------------------------------------------
    @api.constrains('state', 'asset_id', 'asset_fiscal_id')
    def _constrains_check_asset_state(self):
        super(AccountMove, self)._constrains_check_asset_state()
        for move in self.filtered(lambda mv: mv.asset_fiscal_id):
            asset_id = move.asset_fiscal_id
    ...
    def _reverse_fiscal_moves(self, default_values_list=None, cancel=False):
        ...
        for move, default_values in zip(self, default_values_list):
            ...
            if move.asset_id:
                ...
                first_draft = min(move.asset_id.depreciation_fiscal_move_ids.filtered(lambda m: m.state == 'draft'), key=lambda m: m.date, default=None)
                if first_draft:
                    # If there is a draft, simply move/add the depreciation amount here
                    first_draft.depreciation_fiscal_value += move.depreciation_fiscal_value
                else:
                    # If there was no draft move left, create one
                    last_date = max(move.asset_fiscal_id.depreciation_fiscal_move_ids.mapped('date'))
                    method_period = move.asset_fiscal_id.method_fiscal_period

                    self.create(self._prepare_move_for_asset_fiscal_depreciation({
                        'asset_id': move.asset_fiscal_id,
                        'amount': move.depreciation_fiscal_value,
                ...
                msg = _('Depreciation entry %s reversed (%s)') % (move.name, formatLang(self.env, move.depreciation_fiscal_value, currency_obj=move.company_id.currency_id))
                move.asset_fiscal_id.message_post(body=msg)
                default_values['asset_fiscal_id'] = move.asset_fiscal_id.id
                default_values['asset_number_days'] = -move.asset_number_days

        return super(AccountMove, self)._reverse_fiscal_moves(default_values_list, cancel)

    def button_fiscal_cancel(self):
        # OVERRIDE
        res = super(AccountMove, self).button_cancel()
        self.env['account.asset'].sudo().search([('original_fiscal_move_line_ids.move_id', 'in', self.ids)]).write({'active': False})
        return res

    def button_fiscal_draft(self):
        for move in self:
        ...
        return super(AccountMove, self).button_fiscal_draft()

    def _log_depreciation_fiscal_asset(self):
        for move in self.filtered(lambda m: m.asset_id):
            asset = move.asset_id
            msg = _('Depreciation entry %s posted (%s)') % (move.name, formatLang(self.env, move.depreciation_fiscal_value, currency_obj=move.company_id.currency_id))
    ...
    def _auto_create_asset(self):
            ...
            for move_line in move.line_ids:
                if (
                    ...
                    vals = {
                        ...
                        'original_fiscal_move_line_ids': [(6, False, move_line.ids)],
                        'state': 'draft',
                        'acquisition_date': move.invoice_date,
                        'acquisition_fiscal_date': move.invoice_date,
                    }
                    model_id = move_line.account_id.asset_model
                    if model_id:
                        vals.update({
                            'model_id': model_id.id,
                            'model_fiscal_id': model_id.id,
                        })
        ...
        for asset, vals, invoice, validate in zip(assets, create_list, invoice_list, auto_validate):
            if 'model_fiscal_id' in vals:
                asset._onchange_model_fiscal_id()
                if validate:
                    asset.validate()
            if 'model_id' in vals:
    ...
    @api.model
    def _prepare_move_for_asset_fiscal_depreciation(self, vals):
        missing_fields = set(['asset_fiscal_id', 'amount', 'depreciation_beginning_date', 'date', 'asset_number_days']) - set(vals)
        if missing_fields:
            raise UserError(_('Some fields are missing {}').format(', '.join(missing_fields)))
        asset = vals['asset_fiscal_id']
        ...
        partner = asset.original_fiscal_move_line_ids.mapped('partner_id')
        partner = partner[:1] if len(partner) <= 1 else self.env['res.partner']
        move_line_1 = {
            'name': asset.name,
            'partner_id': partner.id,
            'account_id': asset.account_depreciation_fiscal_id.id,
        ...
        move_line_2 = {
            'name': asset.name,
            'partner_id': partner.id,
            'account_id': asset.account_depreciation_expense_fiscal_id.id,
    ...
    def _delete_reversed_entry_fiscal_assets(self):
        ...
        for move in self.filtered(lambda m: m.reversed_entry_id):
            ...
            for line in move.reversed_entry_id.line_ids.filtered(lambda l: (
                ...
                for asset in line.asset_ids:
                    if asset.state == 'draft' or all(state == 'draft' for state in asset.depreciation_fiscal_move_ids.mapped('state')):
                        asset.state = 'draft'
                        asset.unlink()
                ...
                for asset in line.asset_ids:
                    if (
                        ...
                        and (asset.state == 'draft' or all(
                            state == 'draft'
                            for state in asset.depreciation_fiscal_move_ids.mapped('state')
                        ))
                    ):
                        asset.state = 'draft'
                        asset.unlink()
    ...
    def _close_fiscal_assets(self):
        for asset in self.asset_id:
            if asset.asset_type in ('expense', 'sale') and all(m.state == 'posted' for m in asset.depreciation_fiscal_move_ids):
