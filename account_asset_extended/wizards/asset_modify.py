...
class AssetModify(models.TransientModel):
    ...
    method_fiscal_number = fields.Integer(string='Duration', required=True)
    method_fiscal_period = fields.Selection([('1', 'Months'), ('12', 'Years')], string='Number of Months in a Period', help="The amount of time between two depreciations")
    value_fiscal_residual = fields.Monetary(string="Depreciable Amount", help="New residual amount for the asset")
    salvage_fiscal_value = fields.Monetary(string="Not Depreciable Amount", help="New salvage amount for the asset")
    ...
    gain_fiscal_value = fields.Boolean(compute="_compute_gain_fiscal_value")

    account_asset_fiscal_id = fields.Many2one('account.account', string="Gross Increase Account", domain="[('deprecated', '=', False), ('company_id', '=', company_id)]")
    account_asset_counterpart_fiscal_id = fields.Many2one('account.account', domain="[('deprecated', '=', False), ('company_id', '=', company_id)]", string="Asset Counterpart Account")
    account_depreciation_fiscal_id = fields.Many2one('account.account', domain="[('deprecated', '=', False), ('company_id', '=', company_id)]", string="Depreciation Account")
    account_depreciation_expense_fiscal_id = fields.Many2one('account.account', domain="[('deprecated', '=', False), ('company_id', '=', company_id)]", string="Expense Account")
    ...
    gain_or_fiscal_loss = fields.Selection([('gain', 'Gain'), ('loss', 'Loss'), ('no', 'No')], compute='_compute_gain_or_fiscal_loss')
    ...
    def _onchange_action(self):
        res = super(AssetModify, self)._onchange_action()
        if self.modify_action == 'sell' and self.asset_id.children_ids.filtered(lambda a: a.state in ('draft', 'open') or a.value_fiscal_residual > 0):
        ...
        if self.modify_action not in ('modify', 'resume'):
            self.write({'value_fiscal_residual': self.asset_id.value_fiscal_residual, 'salvage_value': self.asset_id.salvage_fiscal_value})
        return res
    ...
    def _compute_gain_or_fiscal_loss(self):
        for record in self:
            balances = abs(sum([invoice.balance for invoice in record.invoice_line_ids]))
            if record.modify_action in ('sell', 'dispose') and record.asset_id.value_fiscal_residual < balances:
                record.gain_or_fiscal_loss = 'gain'
            elif record.modify_action in ('sell', 'dispose') and record.asset_id.value_fiscal_residual > balances:
                record.gain_or_fiscal_loss = 'loss'
            else:
                record.gain_or_fiscal_loss = 'no'
    ...
    def _compute_informational_text(self):
        res = super(AssetModify, self)._compute_informational_text()
        return res
    ...
    def create(self, vals_list):
        for vals in vals_list:
            if 'asset_id' in vals:
                ...
                if asset.depreciation_fiscal_move_ids.filtered(lambda m: m.state == 'posted' and not m.reversal_move_id and m.date > fields.Date.today()):
                    raise UserError(_('Reverse the depreciation entries posted in the future in order to modify the depreciation'))
                if 'method_fiscal_number' not in vals:
                    vals.update({'method_fiscal_number': asset.method_fiscal_number})
                if 'method_fiscal_period' not in vals:
                    vals.update({'method_fiscal_period': asset.method_fiscal_period})
                if 'salvage_fiscal_value' not in vals:
                    vals.update({'salvage_fiscal_value': asset.salvage_fiscal_value})
                if 'value_fiscal_residual' not in vals:
                    vals.update({'value_fiscal_residual': asset.value_fiscal_residual})
                if 'account_asset_fiscal_id' not in vals:
                    vals.update({'account_asset_fiscal_id': asset.account_asset_fiscal_id.id})
                if 'account_depreciation_id' not in vals:
                    vals.update({'account_depreciation_fiscal_id': asset.account_depreciation_fiscal_id.id})
                if 'account_depreciation_expense_fiscal_id' not in vals:
                    vals.update({'account_depreciation_expense_fiscal_id': asset.account_depreciation_expense_fiscal_id.id})
    ...
    def modify(self):
        ...
        old_values = {
            ...
            'method_fiscal_number': self.asset_id.method_fiscal_number,
            'method_fiscal_period': self.asset_id.method_fiscal_period,
            'value_fiscal_residual': self.asset_id.value_fiscal_residual,
            'salvage_fiscal_value': self.asset_id.salvage_fiscal_value,
        }
        ...
        asset_vals = {
            ...
            'method_fiscal_number': self.method_fiscal_number,
            'method_fiscal_period': self.method_fiscal_period,
            'value_fiscal_residual': self.value_fiscal_residual,
            'salvage_fiscal_value': self.salvage_fiscal_value,
        }
        if self.env.context.get('resume_after_pause'):
            ...
            date_before_pause = max(self.asset_id.depreciation_fiscal_move_ids, key=lambda x: x.date).date if self.asset_id.depreciation_fiscal_move_ids else self.asset_id.acquisition_fiscal_date
            number_days = self.asset_id._get_delta_fiscal_days(date_before_pause, self.date) - 1
            if float_compare(number_days, 0, precision_rounding=self.currency_id.rounding) < 0:
                raise UserError(_("No puede reanudar en una fecha igual o anterior a la fecha de pausa"))

            asset_vals.update({'asset_paused_fiscal_days': self.asset_id.asset_paused_fiscal_days + number_days})
            asset_vals.update({'state': 'open'})
            self.asset_id.message_post(body=_("Asset unpaused. %s", self.name))
        ...
        #FISCAL
        current_asset_fiscal_book = self.asset_id.value_fiscal_residual + self.asset_id.salvage_fiscal_value
        after_asset_fiscal_book = self.value_fiscal_residual + self.salvage_fiscal_value
        increase_fiscal = after_asset_fiscal_book - current_asset_fiscal_book

        new_fiscal_residual = min(current_asset_fiscal_book - min(self.salvage_fiscal_value, self.asset_id.salvage_fiscal_value), self.value_fiscal_residual)
        new_fiscal_salvage = min(current_asset_fiscal_book - new_fiscal_residual, self.salvage_fiscal_value)
        residual_fiscal_increase = max(0, self.value_fiscal_residual - new_fiscal_residual)
        salvage_fiscal_increase = max(0, self.salvage_fiscal_value - new_fiscal_salvage)
    ...
    @api.depends('asset_id', 'value_fiscal_residual', 'salvage_fiscal_value')
    def _compute_gain_fiscal_value(self):
        for record in self:
            record.gain_fiscal_value = record.value_fiscal_residual + record.salvage_fiscal_value > record.asset_id.value_fiscal_residual + record.asset_id.salvage_fiscal_value
