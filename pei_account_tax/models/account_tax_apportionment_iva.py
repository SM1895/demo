...
class AccountTaxApportionmentIva(models.Model):
    _name = 'account.tax.apportionment.iva'
    _inherit = ['mail.thread', 'mail.activity.mixin']
    _description = 'Configuración de Categorización de IVA'
    ...
    name = fields.Char(
        string='Categorización IVA  ',
        tracking=True
    )
    accounts_ids = fields.Many2many(
        string='Cuenta Contable',
        relation='apportionment_iva_account_rel',
        column1='apportionment_id',
        column2='account_id',
        comodel_name='account.account',
        tracking=True
    )
    is_discountable = fields.Boolean(
        string='¿Es Descontable?',
        default=False,
        tracking=True
    )
    group_id = fields.Many2one(
        string='Categorización Ingreso',
    ...
    def _check_accounts(self):
        for record in self:
            if record.accounts_ids.ids:
                account_ids = record.accounts_ids.ids 
                account_ids.append(0)
                self._cr.execute(
                    'SELECT count(*) FROM apportionment_iva_account_rel WHERE account_id IN %s AND apportionment_id <> %s' % (
    ...
    def action_read_group(self):
        ...
        return {
            ...
            'view_type': 'form',
            'view_mode': 'form',
            'res_model': 'account.tax.apportionment.iva',
            'res_id': self.id,
        }
